...
if (!empty($idconn) AND !empty($mdpconn)) 
{
...
if (isset($_POST['reguser'])) {
	 //Sécurité contre les injections myslq
	...
	if (!empty($_POST['identifiant']) AND !empty($_POST['email']) AND !empty($_POST['emailconf']) AND !empty($_POST['password_1']) AND !empty($_POST['password_2']))
	{
		...
		if ($identifiantlength <= 255) 
		{
			if ($email == $emailconf) 
			{
				...
				{
					        $reqmail = $db -> prepare("SELECT * FROM utilisateurs WHERE mail = '$email' LIMIT 1");
                  ...
                  $reqmail -> execute(array($email));
					...
					if ($mailexist == 0) 
					{
						if ($password_1 == $password_2) 
						{
					      ...
					      $erreur = "Cette adresse existe déjà";
					    }
	  ...
	  $erreur = "Veuillez bien remplir tout les champs merci";
	}	
